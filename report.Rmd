...
<center>
![](images/nyc_garden_2.JPG){ height="267px" width="200px" }  ![](images/garden1.JPG){ height="267px" width="350px" }  ![](images/radishes.JPG){ height="267px" width="200px" } 
</center>
...
* Connecting community gardens to positive health outcomes, increased neighborhood value, or greater community engagement may inspire more investment in gardens in NYC.  
...
* Demographic & health data from [nyc.gov](https://www1.nyc.gov/site/doh/about/press/pr2019/2018-community-health-profiles.page)
...
* Community District Shapefile from [NYC OpenData](https://data.cityofnewyork.us/City-Government/Community-Districts/yfnk-k7r4)

### *Data Cleaning*

Each text dataset that was obtained was first individually cleaned. Most were already well organized (in xlsx or csv format). Unnecessary variables were removed for efficiency and clarity. Average variables for market value and participatory budget allocation were calculated in these steps as well. 
...
Afterwards, we merged all datasets into two tidied datasets by community board. The first dataset contains one row for each unique community districts with a variable for the number of community gardens per district (`final_unique_boards`), and the second contains one row for each unique community garden (`final_unique_gardens`). At this stage we also obtained spatial data for community districts in shapefile format.
...
The first tidied dataset (`final_unique_boards`) contains 59 observations (the number of community boards in NYC).
The second tidied dataset (`final_unique_gardens`) contains 535 rows (the number of unique community gardens).

* Variables included in `final_unique_boards`  denoted with *
* Variables included in `final_unique_gardens`denotes with **
* Variables existing in both dataset denotes with ***
  ...
  * `poverty`. Percentage of residents living below 100% of New York City’s calculated poverty threshold based on income and necessary expenses in a community board. ***
  * `rent_burden`. Percentage of renter-occupied homes whose gross rent (contract rent plus estimated average monthly cost of utilities including electricity, gas, and water and sewer) is equal to or greater than 30 percent of household income in past 12 months in a community board. ***
  ...
  * `boro`. Borough location of each community board in letter form. ***
  ...
  * `avg_tot_appropriated`. Average amount of participatory budgeting allocated for all projects per a community board. Data last updated February 2020. ***
...
1. A higher number of community gardens in a district will be associated with a lower percentage of obese adults in the district.   
2. A higher number of community gardens in a district will be associated with a lower percentage of adults reporting hypertension in the district.
3. A higher number of community gardens in a district will be associated with a higher average life expectancy in the district.   
4. A higher number of community gardens in a district will be associated with a higher percentage of adults in the district reporting good health.   
5. A higher number of community gardens in a district will be associated with greater total amount of money allocated by the community through participatory budgeting processes for that district.  
6. A higher number of community gardens in a district will be associated with higher average property market values.   

Once we selected our variables of interest and formulated our hypotheses, we did some initial visual explorations to better understand our data. To help answer our first question **"How are community gardens spatially distributed throughout the city?**" we [visualized the distribution of community gardens](main.html) throughout the city. We wanted to see if the locations of community gardens are clustered throughout New York City.
Furthermore, the dashboard for [*Health conditions*] (health.html) provides a visual comparing number of gardens to percent obesity, hypertension, life expectancy, and self reported health. The dashboard for [*Economic investment*] (property_values.html) shows a visual comparison between budget allocated to each board and market value, poverty percent and rent burden percent.
...
# Prepare shapefile for leaflet outline.
...
garden_tidy = 
  left_join(garden_bar_graph, garden_num, by = "community_board")
...
**Notes:** Number of gardens by each community board in NYC (garden data last updated in 2017).

To answer our second question **"How are the locations of gardens related to certain demographic characteristics?"** we created tables showing an overview of demographic information which allows the reader to gain a better understanding of differences between community boards. These tables are side by side with the map of community gardens, so readers can click on a community board on the map and then search for the corresponding board in the tables to see the demographic characteristics of that area.
...
**Notes:** Proportion of age categories in each community board (data collected from 2012 - 2016).
...
**Notes:** Proportion of education levels in each community board (data collected from 2012 - 2016).
...
**Notes:** Proportion of race categories in each community board (data collected from 2012 - 2016).
...
**Notes:** Moran’s I is correlation coefficient that measures the overall spatial auto-correlation among nearby locations in space. It measures how similar an values of a spatial object are to the values of other spatial objects surrounding it. A significant p-value indicates that the gardens are not evenly spatially distributed.

This information serves as a foundation for the other analyses to build off. 

***

### Relationship of Garden Number to Economic and Health Outcomes

The next step in our exploration was to answer the questions **"What is the relationship between community gardens and economic investment?"** and **"What is the relationship between community gardens and health conditions?"** To do this we decided to build regression models using the number of gardens in each community district as the main predictor of interest. We began the process by selecting specific outcome variables that we hypothesized might be influenced by the number of gardens in a community district, informed by prior literature. The studies and articles we reviewed (see Background and Related Work section of the report) suggested that access to green space and gardens was beneficial for health, and that community gardens may also positively impact community engagement and the perceived value of the neighborhood. 


### Health Conditions Dashboard

To visualize the spatial distribution of number of gardens and our [health outcomes](health.html) of interest (hypertension, obesity, life expectancy, and self-reported health), we used leaflet to create choropleths that colored each community district by quartile of value for the variable of interest.
...
**Notes:** Visualization of the distribution of obesity across community boards. Obesity: Percentage of adults ages 18 and older who have obesity (Body Mass Index of 30 or greater) based on self-reported height and weight. Data from the NYC 2018 Community Health Profiles.
...
**Notes:** Visualization of the distribution of hypertension across community boards. Hypertension: Percentage of adults ages 18 and older who report ever being told by a healthcare professional that they have hypertension, also known as high blood pressure. Data from the NYC 2018 Community Health Profiles.
...
**Notes:** Visualization of the distribution of life expectancy across community boards. Life expectancy is defined at birth. Data from the NYC 2018 Community Health Profiles.
...
**Notes:** Visualization of the distribution of self reported health across community boards. Self Reported Health: Percentage of adults ages 18 and older who report their overall health is “excellent,” "very good" or “good” on a scale of excellent, very good, good, fair or poor. Data from the NYC 2018 Community Health Profiles.
...
Interestingly when comparing these health outcomes to the map of garden numbers per community district, it appears that the areas with a greater number of gardens actually have generally worse health outcomes. This is a counter to our initial hypotheses. 

### Economic Conditions Dashboard

To visualize the spatial distribution of number of gardens and the [economic outcomes](property_values.html) of interest, we used leaflet to visualize the locations of gardens and colored these points by quartile of the value for each economic variable of interest. In the dashboard we wanted to show both variables that we think are potential confounders (poverty and rent burden) as well as variables that exemplify community engagement (the amount of money allocated by the community to projects through participatory budgeting) and neighborhood value (average property market value). 

One issue we encountered was that only 33 community districts use participatory budgeting (this is determined by the city council person), so there are about 25 districts where we do not have data on this particular measure of community engagement (visualized by the gray dots). 
...
**Notes:** Visualization of the distribution of community gardens in New York City, colored by participatory budget allocated to each community board. NA values reflect community boards that do not engage in participatory budgeting. Budget allocation: total amount of participatory funding for projects in that community board. Participatory budget: process where community members vote of projects that they want funding to be allocated towards. Data collected starting in 2018 and last updated February 2020. 
...
**Notes:** Visualization of the distribution of community gardens in New York City, colored by average revised market value in each community board, which is updated twice a year (most recently, Feb 2020). Average revised market value: updated original market value.
...
**Notes:** Visualization of the distribution of community gardens in New York City, colored by average percent poverty in each community board location. Poverty: percentage of residents living below 100% of NYC's calculated poverty threshold, based on income and necessary expenses. Data collected from 2012-2016 (community district and NYC).
...
**Notes:** Visualization of the distribution of community gardens in New York City, colored by average percent of monthly income spent on rent in each community board location. Rent burden: percentage of renter-occupied homes whose gross rent is greater than or equal to 30% of the household income in the past 12 months. Data collected from 2012-2016 from the US Census.
...
**Notes:** The above paneled graphs show the distribution of the outcome variables we were interested in studying. 
...
**Notes:** The above paneled plots depict the association of linearity between our exposure (garden number) and covariate (poverty) with all the outcome variables. We dropped market value from our analysis because this association was not close to linear with garden number.
...
**Notes:** The graphic above shows different color and intensity of shading based on the strength of the correlation between variables. We saw that the correlation between our exposure and covariate variables was not very strong, which was desirable. 
...
**Notes:** We wanted to assess the spatial distribution of obesity. A significant p-value was reported, indicating there was uneven spatial distribution.
...
**Notes:** We wanted to assess the spatial distribution of hypertension A significant p-value was reported, indicating there was uneven spatial distribution.
...
**Notes:** We wanted to assess the spatial distribution of life expectancy A significant p-value was reported, indicating there was uneven spatial distribution.
...
Since we only have one observation per community board and the values are only related to each other spatially, we don't have a higher order category to describe the similarity among community boards. Therefore, we chose to run a spatial model instead of a generalized estimating equation (GEE) or multi-level model.

![](images/spatial_model.jpeg){ height="450px" width="400px" }
...
**Spatial Lag Models for Outcomes**
...
# Creating final table to display betas and p-values
...
**Notes:** “Rho”: Indicates the spatial lag parameter (there is no rho term in the self-reported health model because a spatial lag model was not run here). Alpha was set at 0.01 to account for Bonferroni Correction.The yellow highlighted rows draw attention to our main exposure of interest and the green highlighted row indicates the one significant result.
...
```{r, message = FALSE, warning = FALSE}
# Save residuals and predicted values to dataframe
...
**Notes:** The above 2 leaflets depict residuals on the map for the spatial regression. We wanted to explore if after adjusting for the spatial parameter, would remove the spatial correlation of these outcome variables. We made these graphs for all 4 observed outcomes, but only presented 2, as the plots all look similar. 

We also used the Moran's I test to check that the residuals for each model are not spatially correlated, as shown by the insignificant p-values for the Moran's I on the residuals from the all final models. 
...
**Notes:** Table shows **Moran's I** p-values for the final, spatially adjusted models.

***

### Analysis and Visualization Conclusions

Our analysis and visualizations were able to answer some of our initial questions, however we were not able to conclusively answer all of them due to limitations in our data.  

 * Using the visualization of garden locations and Moran's I (p = 0.014) we can conclude that community gardens are not randomly distributed around the city, and are highly concentrated in certain neighborhoods.  

 * We did not do a formal analysis of demographic characteristics and community gardens, and instead opted for a more interactive tool that allows viewers to look up detailed demographic information for each community board. From this we can see that some trends where the areas with more garden tend to have a more racially diverse population and lower education levels. 

 * In our formal analysis through spatial linear regression we set alpha at 0.01 using the Bonferroni Correction, to account for the fact that we are doing multiple comparisons (4 models). Our models reveal that when adjusting for spatial effects and poverty, number of gardens is not associated with health outcomes for obesity, hypertension, or self-reported health. Despite not being significant, in each of these models the coefficient for garden number was in the direction of predicting worse health for each of these outcomes. In the one model where garden number was significant, we found that number of gardens is associated with a 1.19 year decrease in life expectancy when adjusting for poverty and spatial effects. Based on our diagnostics of residuals plotted against predicted values and our residuals plotted spatially, we are satisfied that our models fit the data fairly well. 
 
 * We were not able to analyze the relationships of garden number with participatory budget allocation or average market value because of missing values for these variables. However from the visualization we can see that many of the areas with a high number of gardens also experience high percentage of poverty and high rent burden. Some of the areas with a high concentration of gardens (east Harlem and South Bronx) also show a particularly high average participatory budget allocation. There does not appear to be a trend with market value and garden location.
 
***

## Limitations 

It is important to note the limitations of our model and analysis. 

First, as mentioned we were not able to analyze the economic variables because of missing data. Additionally as we chose to visualize garden points colored by the economic variables, our visualization does not provide much information allowing the comparison of these values between areas of high garden density and low garden density (or areas with no gardens). We chose this visualization method because of the increased detail given with the specific location of gardens, but we recognize the information that is lost given this choice.

Second, the data were not all collected at the same time. For example, the community data was compiled in 2018 but other data were last updated in 2020. Therefore, it is possible that some of the data that was included in this analysis is not up to date. However, we expect that if updated, there would be be significant changes. 

Third, we have a small sample size (59 community boards), which became even smaller after removing one observation for having no neighboring community boards. As a result, we only adjusted for one covariate (poverty) which may not account for all the variance in the model and leave residual confounding. As can be seen from the demographic information section, there are clearly differences in other characteristics between the community boards. These differences may contribute to differential health outcomes, however we were not able to adjust for these which may obscure the true relationship of gardens to health outcomes. 

Finally, when selecting the type of model we are aware that many of our outcomes are not perfectly linear relationships with garden number. Further, many variables that we included in our exploratory visualizations were skewed (ex. poverty, rent burden). We initially tried to use the log of garden number in order to transform to a more linear and normal relationship. However, we ran into issues later when using log(garden number) in the analysis due to the spatial nature. Therefore, we decided to continue with using a linear model, which may not be the absolute best fit for the data.  

***

## Discussion

While we had initially hypothesized that a higher number of gardens in an area would be associated with better health outcomes and economic investment, our analyses and visualizations suggest a largely inverse relationship where more gardens are associated with worse outcomes both health-wise and economically. Our hypothesis was founded on the belief that despite the historic disinvestment in the neighborhoods where community gardens developed, the gardens themselves would serve as features that could improve health. However we do not see this, which could be due to several factors One is that perhaps the entrenched poverty and disinvestment in these areas is so strongly embedded throughout New York City that the relatively small effect of the gardens in a short period of time is not able to be distinguished. Another reason could be that there is residual confounding and garden number is actually capturing a number of other factors that correlate with poor health and poor economic outcomes. As we were only able to adjust for one covariate this is a strong possibility. Another possibility is that we see some reverse causation: as a result of poor health and economic conditions in certain neighborhoods, residents may have been more motivated to develop gardens in order to improve their communities. 

One particularly interesting finding was from the participatory budgeting visualization. Despite the number of missing values in the budget and revised market value variables that precluded formal analysis, the maps show that a few of the areas that have the majority of the gardens (East Harlem and South Bronx) have high amounts of money allocated through participatory budgeting. This indicates that community members voted to allocate more money to community-driven projects than in other areas, which aligns with our hypothesis of greater community engagement in areas with more gardens. It would be interesting to explore this trend further with comparisons to other areas with fewer gardens. 

This project highlights that issues of inequity and divides between boroughs and neighborhoods are still present as ever. The distribution of gardens is not evenly spread across the city, and is concentrated in areas of historic poverty and disenfranchisement, as is expected from the origins of community gardens. Despite our negative findings, community gardens may still have a positive impact on community well-being as prior studies have shown. This is especially likely given the limitations previously discussed of residual confounding. 

Given that community gardens are located primarily in neighborhoods with worse health outcomes, they have the potential to have a large impact on community well-being if they are found to be a benefit to health. Future studies should explore whether or not these gardens have a significant effect on the lives of community members through more individual-focused research. Further it would be interesting to include a variable that measures community engagement, particularly given that the history of these gardens is wrapped up in the activism of community members in these neighborhoods. 

Who knows - maybe in a few decades New York will be known for its gardens rather than its skyscrapers. 
