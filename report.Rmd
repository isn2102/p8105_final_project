...
Afterwards, we merged all datasets into two tidied datasets. The first contains one row for each unique community districts with a variable for the number of community gardens per district, and the second contains one row for each unique community garden. At this stage we also obtained spatial data for community districts in shapefile format.
...
To help answer our first question **"How are community gardens spatially distributed throughout the city?**" we [visualized the distribution of community gardens](main.html) throughout the city. We wanted to see if the locations of community gardens are clustered throughout New York City.
...
Our analysis and visualizations were able to answer some of our initial questions, however we were not able to conclusively answer all of them because of limitations in our data.  
 ...
 * In our formal analysis through spatial linear regression we set alpha at 0.01 to account for the fact that we are doing multiple comparisons (4 models). Our models reveal that when adjusting for spatial effects and poverty, number of gardens is not associated with health outcomes for obesity, hypertension, or self-reported health. Despite not being significant, in each of these models the coefficient for garden number was in the direction of predicting worse health for each of these outcomes. In the one model where garden number was significant, we found that number of gardens is associated with a 1.19 year decrease in life expectancy when adjusting for poverty and spatial effects. Based on our diagnostics of residuals plotted against predicted values and our residuals plotted spatially, we are satisfied that our models fit the data fairly well. 
