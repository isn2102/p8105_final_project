...
It is important to note limitations of our model and analysis. First, we have a small sample size (59 community boards), which became even smaller after removing one observation for having no neighboring community boards, allowing for more variation and spread. As a result, we adjusted for only one covariate, which may not account for all the variance in the model and leave residual confounding. Additionally, we were unable to conduct statistical analyses on the budget or revised market variables because of the number of unavailable values. 


Additionally, when working on the type of model, we initially tried to use the log of garden number in order to normalize the distribution. Further, many variables that we included in our exploratory visualizations were skewed (ex. poverty, rent burden). However, we ran into issues later in the analysis due to the spatial nature. Therefore, we decided to use a linear model, which may not be the best fit. 

***

## Discussion

While we had initially hypothesized that a higher number of gardens in an area would be associated with better health outcomes and economic investment, our analyses and visualizations suggest a largely inverse relationship where more gardens are associated with worse outcomes. This could be due to entrenched poverty and poor health outcomes that have been embedded throughout New York City for decades. As a result, the city may have been more motivated to place gardens in areas with more abandoned lots and in communities to improve health and economic conditions. 

Despite the number of missing values in the budget and revised market value variables, the maps show that a few of the areas that have the majority of the gardens (East Harlem and South Bronx) have high amounts of money allocated through participatory budgeting. This aligns with our hypothesis of greater community engagement in areas with more gardens. 

The garden distribution map shows that gardens are not evenly distributed across the city, with very few in Manhattan. Further, issues of inequity and divides between boroughs and neighborhoods are still present as ever. Future studies may explore the reasoning behind the placement of gardens and whether or not these gardens have a significant effect on the lives of community members. Further, it would be interesting to include a variable that measures community engagement. Given our results, we can see that while gardens have a significant relationship with other factors of life, it is important to continue researching how community gardens are affecting the lives of New Yorkers. 
