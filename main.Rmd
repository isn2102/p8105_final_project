...
number_borough %>%
  ...
  layout(title = "Number of Gardens by Community Board")
```
...
race_white = 
  ...
  ggplot(aes(x = borough, y = proportions,  color = borough)) +
      geom_point() +
      geom_segment(aes(x = borough, xend = borough, y = 0, yend = proportions))
...
race_white = 
  ...
  filter(race == "race_white") %>% 
  group_by(borough, race) %>% 
  mutate(borough = fct_reorder(borough, proportions)) %>% 
  plot_ly(
    x = ~borough, y = ~proportions, color = ~borough,
    type = "bar", colors = "viridis"
  )
