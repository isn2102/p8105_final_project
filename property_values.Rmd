...
property_data %>%
  ...
  addLegend("topleft", pal = pal, values = ~avg_tot_appropriated,
            title = "Budget Allocation </b><br>(in thoundsands)",
...
> Budget allocation: total amount of participatory funding for projects in that community board. Participatory budget: process where community members vote of projects that they want funding to be allocated towards. 
...
property_data %>%
  ...
  addCircleMarkers(~longitude, ~latitude, radius = .1, color = ~pal2(avg_rev_value), popup = ~click_label) %<>%
    addLegend("topleft", pal = pal2, values = ~avg_rev_value, 
               title = "Revised Market Value (Biannually) </b><br>(in thoundsands)",
              opacity = 1,
