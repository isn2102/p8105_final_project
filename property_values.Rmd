...
### Property Market Value by Community Board and Borough
...
ggplotly(market_value)
```   


### Budget Allocation by Borough
    
```{r}
budget_allocation = 
  property_data_final %>% 
    drop_na(avg_tot_appropriated) %>% 
    ggplot(aes(x = borough, y = avg_tot_appropriated, fill = borough)) +
    geom_boxplot() +
    labs(
      title = "Average Budget Allocation",
      x = "Budget Allocation",
      y = "Borough"
    ) +
    theme(legend.position = "none")

ggplotly(budget_allocation)
```

### Percent Poverty by Borough

```{r}
poverty = 
  property_data_final %>% 
    ggplot(aes(x = borough, y = poverty, fill = borough)) +
    geom_boxplot() +
    labs(
...
rent_burden = 
  property_data_final %>% 
    ggplot(aes(x = borough, y = rent_burden, fill = borough)) +
    geom_boxplot() +
    labs(
      title = "Rent Burden",
      x = "Borough",
